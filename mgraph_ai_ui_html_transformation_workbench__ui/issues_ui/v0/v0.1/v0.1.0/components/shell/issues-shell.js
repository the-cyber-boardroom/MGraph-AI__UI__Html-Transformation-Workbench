/**
 * Issues Shell Component
 *
 * Purpose: Main application shell for Issues UI
 * Version: v0.1.0 (Issues UI)
 *
 * Provides the frame for all mini apps with:
...
const SIDEBAR_APPS = ['events-viewer'];
                ...
                <div class="version-switcher">
                    ...
                    <select id="version-select">
                        ${AVAILABLE_VERSIONS.map(v =>
                            `<option value="${v.id}" ${v.id === 'v0.1.0' ? 'selected' : ''}>${v.label}</option>`
                        ).join('')}
                    </select>
                </div>
            </header>
...
console.log('[Issues UI v0.1.0] Shell component initialized');
