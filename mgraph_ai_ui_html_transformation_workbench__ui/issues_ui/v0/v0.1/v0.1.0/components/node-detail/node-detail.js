...
class NodeDetail extends HTMLElement {
    ...
    renderLink(link) {
        ...
        const targetConfig = window.issuesApp.nodeTypes[targetType] || {};
    ...
    attachEventHandlers() {
        ...
        this.querySelector('#nd-modal-overlay')?.addEventListener('click', (e) => {
            if (e.target.id === 'nd-modal-overlay') this.closeLinkModal();
        });

        this.querySelector('#nd-modal-close')?.addEventListener('click', () => this.closeLinkModal());
        this.querySelector('#nd-modal-cancel')?.addEventListener('click', () => this.closeLinkModal());
        ...
        this.querySelector('#nd-modal-create')?.addEventListener('click', () => this.createLink());
    }
    ...
    formatStatus(status) { return status ? status.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) : ''; }
    formatDate(dateStr) {
        if (!dateStr) return '-';
    ...
    getStyles() {
        return `
            .node-detail { height: 100%; background: #1a1a2e; color: #e0e0e0; display: flex; flex-direction: column; }
            .nd-header { display: flex; justify-content: space-between; align-items: center; padding: 12px 20px; border-bottom: 1px solid #2a3f5f; background: #1e2746; }
            .nd-back { background: none; border: none; color: #8a9cc4; cursor: pointer; font-size: 13px; }
            .nd-back:hover { color: #e94560; }
            .nd-actions { display: flex; gap: 8px; }
            .nd-btn { padding: 8px 16px; border: none; border-radius: 4px; font-size: 13px; font-weight: 500; cursor: pointer; }
            .nd-btn-primary { background: #e94560; color: white; }
            .nd-btn-primary:hover:not(:disabled) { background: #d63850; }
            .nd-btn-primary:disabled { background: #3a4f6f; cursor: not-allowed; }
            .nd-btn-secondary { background: #3a4f6f; color: #e0e0e0; }
            .nd-btn-secondary:hover { background: #4a5f7f; }
            .nd-btn-danger { background: transparent; border: 1px solid #ef4444; color: #ef4444; }
            .nd-btn-danger:hover { background: #ef4444; color: white; }
            .nd-btn-small { padding: 4px 10px; font-size: 11px; margin-left: 10px; }
            .nd-content { flex: 1; display: flex; overflow: hidden; }
            .nd-main { flex: 1; overflow-y: auto; padding: 24px; }
            .nd-sidebar { width: 280px; background: #1e2746; border-left: 1px solid #2a3f5f; padding: 20px; overflow-y: auto; }
            .nd-title-row { margin-bottom: 16px; }
            .nd-label-badge { display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; border-radius: 4px; font-size: 12px; font-weight: 600; color: white; }
            .nd-title { margin: 8px 0 0 0; font-size: 24px; font-weight: 600; color: #fff; }
            .nd-meta { display: flex; gap: 16px; align-items: center; flex-wrap: wrap; margin-bottom: 24px; }
            .nd-status-section { display: flex; flex-direction: column; gap: 4px; }
            .nd-status-section label { font-size: 11px; font-weight: 600; color: #8a9cc4; }
            .nd-status-select { padding: 6px 12px; border: 1px solid #3a4f6f; border-radius: 4px; background: #252836; color: #e0e0e0; font-size: 13px; cursor: pointer; }
            .nd-tags { display: flex; gap: 6px; flex-wrap: wrap; }
            .nd-tag { background: #3a4f6f; padding: 4px 10px; border-radius: 4px; font-size: 12px; }
            .nd-section { margin-bottom: 24px; }
            .nd-section h3 { font-size: 14px; font-weight: 600; color: #a0b0c0; margin: 0 0 12px 0; display: flex; align-items: center; }
            .nd-description { background: #252836; padding: 16px; border-radius: 6px; line-height: 1.6; }
            .nd-description p { margin: 0 0 12px 0; }
            .nd-description p:last-child { margin-bottom: 0; }
            .nd-links { background: #252836; padding: 12px; border-radius: 6px; }
            .nd-link-group { margin-bottom: 12px; }
            .nd-link-group:last-child { margin-bottom: 0; }
            .nd-link-group-title { font-size: 11px; font-weight: 600; color: #6a7a8a; text-transform: uppercase; margin-bottom: 8px; }
            .nd-link { display: flex; align-items: center; gap: 8px; padding: 8px; border-radius: 4px; cursor: pointer; margin-bottom: 4px; }
            .nd-link:hover { background: rgba(255, 255, 255, 0.05); }
            .nd-link-verb { font-size: 12px; color: #8a9cc4; min-width: 100px; }
            .nd-link-arrow { color: #4a5a6a; }
            .nd-link-target { flex: 1; font-weight: 500; }
            .nd-link-delete { background: none; border: none; color: #6a7a8a; cursor: pointer; font-size: 16px; padding: 2px 6px; border-radius: 3px; }
            .nd-link-delete:hover { background: #ef4444; color: white; }
            .nd-no-links { text-align: center; color: #6a7a8a; padding: 12px; }
            .nd-sidebar-section { margin-bottom: 20px; }
            .nd-sidebar-section h4 { font-size: 11px; font-weight: 600; color: #6a7a8a; text-transform: uppercase; letter-spacing: 0.5px; margin: 0 0 12px 0; }
            .nd-detail-row { display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #2a3f5f; }
            .nd-detail-label { font-size: 12px; color: #8a9cc4; }
            .nd-detail-value { font-size: 12px; color: #e0e0e0; }
            .nd-mono { font-family: monospace; font-size: 10px; }
            .nd-loading, .nd-error { padding: 60px; text-align: center; }
            .nd-error { color: #e94560; }
            .nd-empty { display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; padding: 40px; text-align: center; }
            ...
            .nd-modal-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.6); display: flex; align-items: center; justify-content: center; z-index: 1000; }
            .nd-modal { background: #1e2746; border-radius: 8px; width: 480px; max-width: 90vw; max-height: 80vh; display: flex; flex-direction: column; }
            .nd-modal-header { display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; border-bottom: 1px solid #2a3f5f; }
            .nd-modal-header h3 { margin: 0; font-size: 16px; color: #fff; }
            .nd-modal-close { background: none; border: none; color: #8a9cc4; font-size: 24px; cursor: pointer; }
            .nd-modal-body { padding: 20px; overflow-y: auto; }
            .nd-modal-footer { padding: 16px 20px; border-top: 1px solid #2a3f5f; display: flex; justify-content: flex-end; gap: 10px; }
            .nd-form-group { margin-bottom: 16px; }
            .nd-form-group label { display: block; font-size: 12px; font-weight: 600; color: #8a9cc4; margin-bottom: 6px; }
            .nd-select, .nd-input { width: 100%; padding: 10px 12px; border: 1px solid #3a4f6f; border-radius: 4px; background: #252836; color: #e0e0e0; font-size: 13px; }
            .nd-select:focus, .nd-input:focus { outline: none; border-color: #e94560; }
            .nd-search-results { background: #252836; border-radius: 4px; max-height: 200px; overflow-y: auto; }
            .nd-search-result { padding: 10px 12px; cursor: pointer; display: flex; flex-direction: column; gap: 2px; border-bottom: 1px solid #2a3f5f; }
            .nd-search-result:last-child { border-bottom: none; }
            .nd-search-result:hover { background: rgba(233, 69, 96, 0.1); }
            .nd-search-result.selected { background: rgba(233, 69, 96, 0.2); }
            .nd-result-label { font-weight: 600; font-size: 12px; }
            .nd-result-title { font-size: 12px; color: #a0b0c0; }
            .nd-no-results { text-align: center; color: #6a7a8a; padding: 20px; }
        `;
    }
    ...
    get markdown() { return window.issuesApp.markdown; }
}
