...
class CreateNodeModal extends HTMLElement {
    ...
    attachEventHandlers() {
        this.querySelector('#cnm-overlay')?.addEventListener('click', (e) => {
            if (e.target.id === 'cnm-overlay') this.close();
        });

        this.querySelector('#cnm-close')?.addEventListener('click', () => this.close());
        this.querySelector('#cnm-cancel')?.addEventListener('click', () => this.close());
        ...
        this.querySelector('#cnm-create')?.addEventListener('click', () => this.create());
    ...
    getStyles() {
        return `
            .cnm-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.6); display: flex; align-items: center; justify-content: center; z-index: 1000; }
            .cnm-modal { background: #1e2746; border-radius: 8px; width: 520px; max-width: 90vw; max-height: 85vh; display: flex; flex-direction: column; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.4); }
            .cnm-header { display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; border-bottom: 1px solid #2a3f5f; }
            .cnm-header h3 { margin: 0; font-size: 16px; color: #fff; }
            .cnm-close { background: none; border: none; color: #8a9cc4; font-size: 24px; cursor: pointer; padding: 0; line-height: 1; }
            .cnm-close:hover { color: #e94560; }
            .cnm-body { padding: 20px; overflow-y: auto; }
            .cnm-footer { padding: 16px 20px; border-top: 1px solid #2a3f5f; display: flex; justify-content: flex-end; gap: 10px; }
            .cnm-error { background: rgba(239, 68, 68, 0.2); border: 1px solid #ef4444; color: #ef4444; padding: 10px 14px; border-radius: 4px; margin-bottom: 16px; font-size: 13px; }
            .cnm-form-group { margin-bottom: 16px; }
            .cnm-form-group label { display: block; font-size: 12px; font-weight: 600; color: #8a9cc4; margin-bottom: 6px; }
            .cnm-required { color: #e94560; }
            .cnm-form-row { display: flex; gap: 16px; }
            .cnm-form-half { flex: 1; }
            .cnm-type-selector { display: flex; gap: 8px; flex-wrap: wrap; }
            .cnm-type-btn { padding: 8px 14px; border: 1px solid #3a4f6f; border-radius: 4px; background: transparent; color: #a0b0c0; font-size: 13px; cursor: pointer; transition: all 0.15s; }
            .cnm-type-btn:hover { background: rgba(255, 255, 255, 0.05); border-color: #4a5f7f; }
            .cnm-type-btn.active { background: var(--type-color, #3a4f6f); border-color: var(--type-color, #3a4f6f); color: white; }
            .cnm-input, .cnm-select, .cnm-textarea { width: 100%; padding: 10px 12px; border: 1px solid #3a4f6f; border-radius: 4px; background: #252836; color: #e0e0e0; font-size: 13px; font-family: inherit; }
            .cnm-textarea { resize: vertical; min-height: 80px; }
            .cnm-input:focus, .cnm-select:focus, .cnm-textarea:focus { outline: none; border-color: #e94560; }
            .cnm-btn { padding: 10px 20px; border: none; border-radius: 4px; font-size: 13px; font-weight: 500; cursor: pointer; transition: all 0.15s; }
            .cnm-btn-primary { background: #e94560; color: white; }
            .cnm-btn-primary:hover:not(:disabled) { background: #d63850; }
            .cnm-btn-primary:disabled { background: #3a4f6f; cursor: not-allowed; }
            .cnm-btn-secondary { background: #3a4f6f; color: #e0e0e0; }
            .cnm-btn-secondary:hover { background: #4a5f7f; }
        `;
    }
