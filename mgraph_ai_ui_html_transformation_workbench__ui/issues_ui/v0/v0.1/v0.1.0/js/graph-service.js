 ...
 * - Nodes by type: /nodes/api/nodes/type/{node_type}
 * - Links: /links/api/nodes/{label}/links
 * - Types: /types/api/types/{name}
 * - Link Types: /types/api/link-types/{verb}
 *
...
(function() {
    ...
    // Correct API base paths from openapi.json
    const API_PATHS = {
        nodes: '/nodes/api/nodes',
    ...
    class GraphService {
        ...
        async listNodes() {
            try {
                const response = await this._fetch(API_PATHS.nodes);
        ...
        async listNodesByType(nodeType) {
            try {
                const response = await this._fetch(`${API_PATHS.nodesByType}/${nodeType}`);
        ...
        async getNode(label) {
            ...
            try {
                const node = await this._fetch(`${API_PATHS.nodes}/${label}`);
        ...
        async updateNode(label, data) {
            try {
                const response = await this._fetch(`${API_PATHS.nodes}/${label}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(data)
                });
        ...
        async listLinks(label) {
            try {
                const response = await this._fetch(`${API_PATHS.links}/${label}/links`);
        ...
        async createLink(sourceLabel, verb, targetLabel) {
            try {
                const response = await this._fetch(`${API_PATHS.links}/${sourceLabel}/links`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        async deleteLink(sourceLabel, targetLabel) {
            try {
                const response = await this._fetch(`${API_PATHS.links}/${sourceLabel}/links/${targetLabel}`, {
                    method: 'DELETE'
                });
    ...
    // Initialize issuesApp if needed
    ...
    window.issuesApp.nodeTypes = NODE_TYPES;
    window.issuesApp.linkTypes = LINK_TYPES;

    console.log('[Issues UI v0.1.0] Graph service initialized with correct API paths');
