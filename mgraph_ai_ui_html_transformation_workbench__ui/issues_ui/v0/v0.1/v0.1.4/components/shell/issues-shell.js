/**
 * Issues Shell Component - v0.1.4
 *
 * Purpose: Main application shell for Issues UI with graph visualization and messages
 * Version: v0.1.4
 *
 * Changes from v0.1.3:
 * - Added Messages Panel to sidebar tabs
 * - Added Graph Viewer as main app
 * - Updated version switcher with v0.1.4
 * - Added message count badge to sidebar toggle
 *
...
const AVAILABLE_VERSIONS = [
    ...
    { id: 'v0.1.4', label: 'v0.1.4 (Graph Viz)' }
];
...
class IssuesShell extends HTMLElement {
    ...
    connectedCallback() {
        ...
        console.log('[Issues UI v0.1.4] Shell initialized with Graph Viewer, Messages Panel');
    }
    ...
    render() {
        this.innerHTML = `
            <style>
                ...
                /* v0.1.4: Message badge in sidebar toggle */
                .sidebar-toggle {
                    position: relative;
                }
                .sidebar-toggle .msg-badge {
                    position: absolute;
                    top: -6px;
                    right: -6px;
                    background: #ef4444;
                ...
                .sidebar-toggle .msg-badge.hidden {
                    display: none;
                }
                /* v0.1.4: Messages tab styling */
                ...
                <div class="version-switcher">
                    ...
                    <select id="version-select">
                        ${AVAILABLE_VERSIONS.map(v =>
                            `<option value="${v.id}" ${v.id === 'v0.1.4' ? 'selected' : ''}>${v.label}</option>`
                        ).join('')}
                    </select>
                </div>
            </header>
        ...
        this.$msgBadge = this.querySelector('#msg-badge');
    ...
    setupEventListeners() {
        ...
        // v0.1.4: Message events for badge update
        this._boundHandlers.onMessageAdded = this.updateMessageBadge.bind(this);
        this.events.on('message-added', this._boundHandlers.onMessageAdded);
        this.events.on('message-dismissed', this._boundHandlers.onMessageAdded);
        this.events.on('messages-cleared', this._boundHandlers.onMessageAdded);
    ...
    // v0.1.4: Update message badge
    updateMessageBadge() {
        if (!this.$msgBadge || !window.issuesApp.messages) return;

        const count = window.issuesApp.messages.getActiveCount();
        const errorCount = count.error;

        if (errorCount > 0) {
            this.$msgBadge.textContent = errorCount;
            this.$msgBadge.classList.remove('hidden');
        } else {
            this.$msgBadge.classList.add('hidden');
        }

        // Update messages tab styling
        const msgsTab = this.querySelector('.sidebar-tab[data-tab="messages-panel"]');
        if (msgsTab) {
            msgsTab.classList.toggle('has-errors', errorCount > 0);
            // Update or create badge
            let badge = msgsTab.querySelector('.tab-badge');
            if (errorCount > 0) {
                if (!badge) {
                    badge = document.createElement('span');
                    badge.className = 'tab-badge';
                    msgsTab.appendChild(badge);
                }
    ...
    handleHashRoute() {
        ...
        console.log('[v0.1.4] Hash route:', hash);
        ...
        if (hash.startsWith('/issue/')) {
            ...
            // v0.1.4: Graph route
            const label = hash.split('/')[2];
            if (label && this._appInstances.has('graph-viewer')) {
                const nodeType = label.split('-')[0].toLowerCase();
    ...
    onNavigated(detail) {
        ...
        // graph-viewer hash is set when graph is loaded
    }
    ...
    toggleSidebar() {
        if (this._sidebarOpen) {
            ...
            // v0.1.4: Default to messages panel if there are errors
            const errorCount = window.issuesApp.messages?.getActiveCount().error || 0;
            const defaultTab = errorCount > 0 ? 'messages-panel' : (this._activeSidebarTab || 'events-viewer');
            this.openSidebarTab(defaultTab);
        }
    }
